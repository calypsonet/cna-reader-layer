...
//	    IReaderModule pluginModuleToCertify = new StubReaderModule();
        ...
        try {
            ...
            Console.display("Ensure that the Reader Layer does not send a Select command (P2=02h) "
                            + "with a DF name lower than 5Bytes ");
            ...
            if (CardAvailable) {
                ...
                Console.display("Process a card selection using AID with length to 4 bytes and P2 = 02h");
                Console.display("AID: " + AID_For_Test);
                RLP.RL_P_UT_SmartCardSelection_With_Occurence(AID_For_Test, "NEXT");
            }
            else
