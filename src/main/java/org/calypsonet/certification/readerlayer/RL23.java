...
//	    IReaderModule pluginModuleToCertify = new StubReaderModule();
        ...
        try {
            ...
            Console.display("Ensure that the Reader Layer provide another way than sending an APDU "
                    + "command to check the presence");
            ...
            if (CardAvailable) {
                ...
                CardAvailable = RLP.RL_P_UT_CheckCardPresence();
                if (!CardAvailable) {
                    Console.display("Card is not detected for the reader: " + RLP.RL_P_UT_GetReaderName());
                }
                else
                    ErrorMessage = "Card still detected  for the reader "+ RLP.RL_P_UT_GetReaderName();
            }
            else
